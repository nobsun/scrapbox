...
solve :: Solver
solve lls = catMaybes $ snd $ mapAccumL phi S.empty lls where
    phi s = \ case
        l | S.member l s -> (s, Nothing)
          | otherwise    -> (S.insert l s, Just l)
...
main :: IO ()
main = B.interact (encode . fromCodom . solve . toDom . decode)
